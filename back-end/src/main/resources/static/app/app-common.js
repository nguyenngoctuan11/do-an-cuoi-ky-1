...
export function currentUser(){
  ...
  const roles = rawRoles.split(',').map(s=>s.trim().toUpperCase()).filter(Boolean);
...
function adminLinksForRoles(roles){
  ...
  if(canTeach){
    items.push({ id:'teacher-courses', href:'/app/admin/teacher-courses.html', label:'Khóa học của tôi', hint:'Theo dõi bản nháp & trạng thái' });
...
export function nav(variant='default'){
  ...
  const who = email
    ? `<div class="who-email">${email}${roles.length?` <span class="muted">[${roles.join(', ')}]</span>`:''}</div>
  ...
  return `
    <header class="topbar ${variant==='compact'?'topbar-compact':''}">
      ...
      <nav class="menu">
        ${baseLinks.map(l=>`<a href="${l.href}" class="${currentPath===l.href?'active':''}">${l.label}</a>`).join('')}
        ${adminLinks.map(l=>`<a href="${l.href}" class="${currentPath===l.href?'active':''}">${l.label}</a>`).join('')}
      </nav>
...
export function mountNav(targetId='nav'){
  ...
  if(!container) return;
  const variant = container.dataset && container.dataset.variant ? container.dataset.variant : 'default';
  container.innerHTML = nav(variant);
...
export function mountAdminMenu(targetId='adminMenu', activeId){
  ...
  host.innerHTML = links.map(link=>{
    const isActive = key === link.id || location.pathname === link.href || key === link.href;
